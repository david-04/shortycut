...
class DictionaryItem {
    ...
    private initializeChildren() {
        ...
        this.shortcuts.forEach(shortcut => {
            if (this.level < shortcut.keyword.length) {
                ...
                dictionary
                    .computeIfAbsent(letter, () => new DictionaryItem(this.level + 1, []))
    ...
    private calculateChildSuggestions(maxResults: number, shortcuts: Shortcut[]) {
        ...
        for (const shortcut of shortcuts) {
            for (const match of shortcut.getSegmentMatches(this.level)) {
                ...
                matches
                    .computeIfAbsent(match.fingerprint, () => {
                        count++;
    ...
    private createChildSuggestion(match: MatchingSegment, shortcuts: Shortcut[]): Suggestion {
        ...
        const descriptionHtmlSuffix =
            !match.isPartial && shortcuts[0]?.searchable
        ...
        return {
            ...
            shortcut: assertNotNull(shortcuts[0]),
            ...
            state.shortcuts.values.sort(
                comparing(
                    s =>
                        ...
                        s.queries?.current[0]?.segments.description ??
                        ""
                )
            )
        );
